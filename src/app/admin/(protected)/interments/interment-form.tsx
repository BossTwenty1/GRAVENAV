...
export function IntermentForm({
  ...
  return (
    <form action={formAction} className="grid gap-7" noValidate ref={formRef}>
      <section aria-labelledby="deceased-picker-heading" className="admin-form-section">
        ...
        {selectedDeceased ? (
          <div className="admin-selected-record mt-3">
            ...
            <button className="admin-text-link mt-2" disabled={pending} onClick={() => setSelectedDeceased(null)} type="button">Change selection</button>
          </div>
        ) : (
          <div className="mt-3 grid gap-3">
            <div className="flex flex-col gap-2 sm:flex-row">
              <label className="sr-only" htmlFor="deceased-picker-search">Search deceased records</label>
              <input autoComplete="off" className="admin-control min-w-0 flex-1" id="deceased-picker-search" maxLength={100} placeholder="Search deceased name…" type="search" />
              <button className="admin-button-secondary" disabled={deceasedPending || pending} onClick={() => {
            ...
            {deceasedResults.length ? <ul aria-live="polite" className="grid gap-2">{deceasedResults.map((option) => (
              <li className="flex flex-col justify-between gap-2 rounded-lg border bg-white p-3 sm:flex-row sm:items-center" key={option.id}>
                ...
                <button className="admin-button-secondary" onClick={() => setSelectedDeceased(option)} type="button">Select record</button>
              </li>
            ))}</ul> : null}
          </div>
        )}
      ...
      <section aria-labelledby="plot-picker-heading" className="admin-form-section">
        ...
        {selectedPlot ? (
          <div className="admin-selected-record mt-3">
            ...
            <button className="admin-text-link mt-2" disabled={pending} onClick={() => setSelectedPlot(null)} type="button">Change selection</button>
          </div>
        ) : (
          <div className="mt-3 grid gap-3">
            <div className="flex flex-col gap-2 sm:flex-row">
              <label className="sr-only" htmlFor="plot-picker-search">Search plots</label>
              <input autoComplete="off" className="admin-control min-w-0 flex-1" id="plot-picker-search" maxLength={100} placeholder="Search plot identifier…" type="search" />
              <button className="admin-button-secondary" disabled={plotPending || pending} onClick={() => {
            ...
            {plotResults.length ? <ul aria-live="polite" className="grid gap-2">{plotResults.map((option) => (
              <li className="flex flex-col justify-between gap-2 rounded-lg border bg-white p-3 sm:flex-row sm:items-center" key={option.id}>
                ...
                <button className="admin-button-secondary" onClick={() => setSelectedPlot(option)} type="button">Select plot</button>
              </li>
            ))}</ul> : null}
          </div>
        )}
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          ...
          <input aria-describedby={state.fieldErrors?.intermentDate ? "intermentDate-error" : undefined} aria-invalid={Boolean(state.fieldErrors?.intermentDate)} autoComplete="off" className="admin-control mt-2" defaultValue={values.intermentDate} disabled={pending} id="intermentDate" name="intermentDate" type="date" />
        ...
        <div>
          ...
          <input aria-describedby={state.fieldErrors?.positionSequence ? "positionSequence-error" : undefined} aria-invalid={Boolean(state.fieldErrors?.positionSequence)} autoComplete="off" className="admin-control mt-2" defaultValue={values.positionSequence} disabled={pending} id="positionSequence" min="1" name="positionSequence" step="1" type="number" />
        ...
        <div>
          ...
          <input aria-describedby={state.fieldErrors?.intermentType ? "intermentType-help intermentType-error" : "intermentType-help"} aria-invalid={Boolean(state.fieldErrors?.intermentType)} autoComplete="off" className="admin-control mt-2" defaultValue={values.intermentType} disabled={pending} id="intermentType" maxLength={100} name="intermentType" type="text" />
        ...
        <div>
          ...
          <input aria-describedby={state.fieldErrors?.permanenceStatus ? "permanenceStatus-error" : undefined} aria-invalid={Boolean(state.fieldErrors?.permanenceStatus)} autoComplete="off" className="admin-control mt-2" defaultValue={values.permanenceStatus} disabled={pending} id="permanenceStatus" maxLength={100} name="permanenceStatus" type="text" />
      ...
      {state.duplicate ? <p className="admin-alert-error" role="alert">Review the existing interment for {state.duplicate.deceasedName} dated {formatDate(state.duplicate.intermentDate)}. Exact duplicates cannot be deliberately bypassed. <Link className="font-semibold underline" href={`/admin/interments/${state.duplicate.id}`} rel="noopener noreferrer" target="_blank">Open record</Link></p> : null}

      {state.occupancyWarning ? (
        <section className="admin-alert-warning" role="alert">
      ...
      <div className="flex flex-wrap gap-3 border-t pt-5">
        <button className="admin-button-primary" disabled={pending || deceasedPending || plotPending} type="submit">{pending ? "Saving…" : state.occupancyWarning ? "Confirm and save interment" : submitLabel}</button>
